...
class RAGAgent:
    ...
    def _split_document(self, content: str, title: str, note_id: str) -> List[str]:
        ...
        for para in paragraphs:
            ...
            if current_token_count + para_token_count > self.config.CHUNK_SIZE and current_chunk:
                # 当前块已满，保存并开始新块
    ...
    def _force_split_chunk(self, text: str) -> List[str]:
        ...
        while start < text_length:
            ...
            if self.config.CHUNK_OVERLAP > 0:
                start = max(start - self.config.CHUNK_OVERLAP, start)  # 确保不会倒退到start位置之前
    ...
    def run_cli(self):
        ...
        运行命令行交互界面
        """
        print("RAG智能体已启动！输入 'quit' 或 'exit' 退出，输入 'reindex' 重新索引笔记。")
