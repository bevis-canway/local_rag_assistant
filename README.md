...
- **多知识库支持**：支持动态配置和管理多个知识库，可跨库检索相关信息
...
### 多知识库支持
小魔仙RAG智能体现在支持管理多个独立的知识库：
- **动态配置**：支持运行时添加、移除和配置多个知识库
- **跨库检索**：查询时自动跨所有启用的知识库进行检索
- **独立存储**：每个知识库有独立的向量存储
- **灵活管理**：支持启用/禁用特定知识库
- **自动发现**：自动扫描指定目录下的所有子目录作为独立知识库
- **CLI管理**：通过命令行界面轻松管理多个知识库
- **弹性索引**：支持对所有知识库或指定知识库进行索引
